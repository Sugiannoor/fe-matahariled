...
import {
  useHistoryUser,
} from "@/features/history/api/getHistoryUser";
      ...
      {data.map((history: HistoryUser) => (
        <Carousel.Slide key={history.history_id} className="flex flex-col">
          <Center className="justify-center my-auto flex-col">
            ...
            <div onClick={()=> navigate(`/portofolio/${history.history_id}`)} className="cursor-pointer">
            <img
              src={import.meta.env.VITE_API_URL + `${history.user.photo}` ?? "/user_default.png"}
              alt="User logo"
              className="mb-4 h-36 object-cover rounded-lg"
              />
              </div>
            <p className="text-2xl lg:text-5xl font-bold text-orange-500">
              {history.title}
            </p>
